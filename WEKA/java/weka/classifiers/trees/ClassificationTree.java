...
public class ClassificationTree extends AbstractClassifier implements AdditionalMeasureProducer {
    ...
    private class InternalNode implements Node, Serializable {
	public Splitter splitter;
	public Node leftSuccessor;
	public Node rightSuccessor;
	public InternalNode(Splitter s, Node l, Node r) {
	    splitter = s; leftSuccessor = l; rightSuccessor = r;
	}
    }
    private class LeafNode implements Node, Serializable {
	public double[] classProbabilities;
	public LeafNode(double[] p) {
    ...
    /** Whether to plot split. */
    private boolean plotSplit = false;
    ...
    /**
     * Method to set whether splits should be plotted.
     */
    @OptionMetadata(displayName = "plot split", description = "Whether to plot split (default = false).",
		    commandLineParamName = "plot-split", commandLineParamSynopsis = "-plot-split", displayOrder = 2, 	commandLineParamIsFlag = true)
    public void setPlotSplit(boolean b) {
       plotSplit = b;
    }
    public boolean getPlotSplit() {
	return plotSplit;
    }
	...
	splitter.findSplit(data);
	    ...
	    if (plotSplit && splitter instanceof BivariateNumericAttributeSplitter) {
		((BivariateNumericAttributeSplitter)splitter).plot(data, getNumDecimalPlaces());
	    }
            Instances leftSubset = new Instances(data, data.numInstances());
            Instances rightSubset = new Instances(data, data.numInstances());
            for (Instance instance : data) {
